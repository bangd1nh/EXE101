...
function PhotographerProfile() {
  ...
  const [selectedMajor, setSelectedMajor] = useState(photographerProfile.major[0]);
    ...
    <>
      <div className="w-7xl mx-auto p-8 rounded-lg bg-white grid grid-cols-4 gap-4">
        <div className="flex justify-center items-center">
          <img src={photographer.PhotographerId.Avatar} alt={photographer.PhotographerId.Username} className="w-80 h-80 rounded-lg object-cover" />
        </div>

        <div className="col-span-2">
          <h1 className="text-3xl font-semibold text-blue-600">
            {photographer.PhotographerId.FirstName} {photographer.PhotographerId.LastName}
          </h1>
          <p className="text-gray-600">{photographer.Location}</p>
          <p className="mt-2 text-base font-light">{photographer.Description}</p>
          <p className="mt-2 text-base font-light">Device: {photographer.Device}</p>
          <div className="grid grid-cols-1 gap-4 mt-4 text-base text-gray-500">
            <div className="flex items-center gap-2">
              <ClockCircleOutlined /> <span>Tham gia vào {photographerProfile.joiningAt}</span>
            </div>
            <div className="flex items-center gap-2">
              <GlobalOutlined /> <span>Ngôn ngữ: {photographerProfile.language}</span>
            </div>
          </div>
          <div className="flex flex-col gap-3 mt-4">
            {photographerProfile.award.map((award, index) => (
              ...
              >
                <StarOutlined className="text-yellow-500" /> {award}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col items-stretch gap-4 ">
          <div className="flex justify-between">
            <div className="">
              <p className="text-base font-extralight">{photographerProfile.follower}</p>
              <p className="text-base font-extralight">Người theo dõi</p>
            </div>
          ...
          <Button variant="outlined" className="border px-8 py-3 rounded-lg flex items-center gap-2 w-full" startIcon={<WhatsAppOutlined />}>
            WhatsApp
          </Button>
          <a href={photographerProfile.website} className="mt-4 block text-blue-600 underline text-center">
          ...
          return (
            <button
              ...
              className={`px-4 py-2  rounded-xl uppercase font-semibold hover:bg-gray-400 duration-300 ${
                selectedMajor.name === m.name ? "bg-stone-700 text-white" : "bg-gray-200 text-black"
              }`}
      ...
      <div className="text-center mt-5">
        ...
        <div className="max-w-6xl mx-auto px-10 mt-20">
          <div className="grid grid-cols-4 gap-4">
            {photographer.PhotoGraphs?.imgUrl?.length > 0 ? (
              <div className="grid grid-cols-4 gap-4">
                {photographer.PhotoGraphs.imgUrl.map((p, index) => (
                  <div key={index} className="relative group">
                    <img className="h-64 w-64 object-cover rounded-2xl" src={p} alt={`Gallery ${index}`} />
              ...
              <p className="text-gray-500 italic text-sm mt-4">Photographer chưa có ảnh portfolio.</p>
            )}
          </div>
        </div>
            ...
            .map((_, index) => {
              return (
                <button
                  key={index}
                  className={
                    `hover:cursor-pointer border px-2 rounded  hover:border-black duration-300 ` + (index === 0 ? "border-black" : "border-gray-300")
                  }
                >
                  {index}
                </button>
